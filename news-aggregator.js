...
const fallbackArticles = [
  {id:'fallback-boc',title:'Bank of Canada feed unavailable fallback',source:'Bank of Canada',url:'https://www.bankofcanada.ca/rss-feeds/',link:'https://www.bankofcanada.ca/rss-feeds/',category:'Rates',tier:'Tier 1',relevance:5,impact:'Unclassified',why:'Check source manually. Rate and monetary policy news may affect borrowing conditions and housing demand.',summary:''},
  {id:'fallback-cmhc',title:'CMHC feed unavailable fallback',source:'CMHC',url:'https://www.cmhc-schl.gc.ca/media-newsroom/cmhc-news-room-rss',link:'https://www.cmhc-schl.gc.ca/media-newsroom/cmhc-news-room-rss',category:'Housing',tier:'Tier 1',relevance:5,impact:'Unclassified',why:'Check source manually. Housing news may affect starts, supply, affordability, and absorption assumptions.',summary:''}
];
...
function normalizeArticle(a){return {id:a.id||('article-'+Date.now()+Math.random()),title:a.title||'Untitled article',source:a.source||'Unknown source',url:a.url||a.link||'',link:a.link||a.url||'',category:a.category||'General',tier:a.tier||'Source',relevance:Number(a.relevance||3),impact:a.impact||'Unclassified',why:a.why||'Review this source for possible Chilliwack land development implications.',summary:a.summary||'',published:a.published||null}}
async function loadLiveNews(){
  ...
  try{
    ...
    if(!articles.length){ articles = fallbackArticles; toast('No live articles returned. Showing fallback sources.'); }
    ...
    $('lastUpdated').textContent = data.generatedAt ? 'Last updated: '+new Date(data.generatedAt).toLocaleString() : 'Last updated just now';
    ...
    articles = stored || fallbackArticles;
...
function classify(article,impact){article.impact=impact; reviewed(article); localStorage.setItem('ceiAggregatorArticlesLive',JSON.stringify(articles)); toast('Marked '+impact); render()}
...
function openSource(article){ const u=article.url||article.link; if(u){window.open(u,'_blank','noopener')} else toast('No source URL') }
function renderSourceStatus(){
 const wrap=$('sourceStatus'); wrap.innerHTML='';
 if(!sourceStatus.length){wrap.innerHTML='<div class="empty">No source status yet.</div>'; return;}
 sourceStatus.forEach(s=>{const div=document.createElement('div');div.className='source-row '+(s.ok?'ok':'bad');div.innerHTML=`<strong>${s.source}</strong><span>${s.ok?'OK':'Issue'} · ${s.count||0} articles${s.error?' · '+s.error:''}</span>`;wrap.appendChild(div)});
}
function render(){
 renderSourceStatus();
...
function addManual(){const title=$('manualTitle').value.trim(); if(!title){toast('Add a title first');return} const article={id:'manual-'+Date.now(),title,source:$('manualSource').value.trim()||'Manual source',url:$('manualUrl').value.trim(),link:$('manualUrl').value.trim(),category:$('manualCategory').value,tier:'Manual',relevance:4,impact:'Unclassified',why:$('manualWhy').value.trim()||'Manual interpretation not added yet.',summary:'',published:new Date().toISOString()}; articles.unshift(article); localStorage.setItem('ceiAggregatorArticlesLive',JSON.stringify(articles)); ['manualTitle','manualSource','manualUrl','manualWhy'].forEach(id=>$(id).value=''); toast('Article added'); render()}
